...
const Leaderboard = () => {
  ...
  const filtered = leaders.filter((item) => JSON.stringify(item).toLowerCase().includes(search.toLowerCase()));
            ...
            <tbody>
              {filtered.map((item, idx) => (
                <tr key={item.id || idx}>
                  ...
                  <td>{toText(item.name || item.user || item.username || 'Competitor')}</td>
                  <td>{toText(item.score || item.points || item.total || '-')}</td>
            ...
            <div className="modal-dialog modal-lg modal-dialog-centered" role="document">
              <div className="modal-content">
                <div className="modal-header">
                  <h3 className="modal-title h5">Leaderboard Details</h3>
